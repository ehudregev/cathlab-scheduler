...
def generate_schedule(year, month, db, Doctor, Request, ScheduleEntry, HistoryEntry):
    ...
    for d in weekday_oncall_days_all:
        ...
        # Priority 4: cap = 3/week, non-weekend-week doctors only
        if not pool:
            pool = [
                doc for doc in oncall_doctors
                if date_str not in unavailable[doc.id]
        ...
        # Priority 5: any weekday count, non-weekend-week doctors only
        if not pool:
            pool = [
                doc for doc in oncall_doctors
                if date_str not in unavailable[doc.id]
        ...
        # Priority 6: last resort — include weekend-week doctors (any weekday count)
        if not pool:
            pool = [
                doc for doc in oncall_doctors
                if date_str not in unavailable[doc.id]
    ...
    for day in session_days:
        ...
        def sort_key(doc):
            ...
            is_preferred = date_str in preferred_session[doc.id]
            return (
                -remaining,                                                   # most sessions still needed = highest priority
                -is_preferred,                                                # preferred day → strong boost (2nd priority)
